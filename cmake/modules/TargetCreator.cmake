...
install(
  DIRECTORY ${PROJECT_BINARY_DIR}/${CMAKE_INSTALL_INCLUDEDIR}/${PROJECT_NAME}
...
function(map_src_to_generated GENERATED INPUT)
  ...
  if(GEN_DIR)
    ...
    set(${GENERATED}
        ${OUTPUT_DIR}/${GEN_DIR}/${GEN_SRC_NAME}
        PARENT_SCOPE
        )
  else()
    set(${GENERATED}
        ${OUTPUT_DIR}/${GEN_SRC_NAME}
        PARENT_SCOPE
        )
  endif()
endfunction()
...
function(default_test_properties)
  set(vars_to_set CMAKE_SYSTEM=${CMAKE_SYSTEM} CMAKE_SYSTEM_PROCESSOR=${CMAKE_SYSTEM_PROCESSOR})
  if(LINUX)
    list(APPEND vars_to_set CMAKE_LIBRARY_ARCHITECTURE=${CMAKE_LIBRARY_ARCHITECTURE})
  endif()
  set_tests_properties(${ARGN} PROPERTIES FAIL_REGULAR_EXPRESSION "[Tt]est [Ff]ailed;TEST FAILED" ENVIRONMENT "${vars_to_set}")
endfunction()
...
function(add_targets_tests target)
  ...
  if(CMAKE_PROJECT_NAME STREQUAL PROJECT_NAME AND BUILD_TESTING)
    ...
    if(DEFINED GIT_COMMIT_HASH)
      ...
      file(
        GLOB CANDIDATE_TARGET_TESTS
        LIST_DIRECTORIES false
        RELATIVE ${CMAKE_CURRENT_SOURCE_DIR}
        CONFIGURE_DEPENDS "${CMAKE_CURRENT_SOURCE_DIR}/${target}_test.[cfF]9?0?"
        )
      file(
        GLOB CANDIDATE_UNIT_TESTS
        LIST_DIRECTORIES false
        RELATIVE ${CMAKE_CURRENT_SOURCE_DIR}/test
        CONFIGURE_DEPENDS "${CMAKE_CURRENT_SOURCE_DIR}/${target}test/*_test.[cfF]9?0?"
        )
    else()
      file(
        GLOB CANDIDATE_TARGET_TESTS
    ...
    foreach(test IN LISTS CANDIDATE_TARGET_TESTS)
      ...
      if(${target_name_wle}_test STREQUAL ${test_name})
        ...
        set_tests_properties(${test_name} PROPERTIES LABELS ${target})
      ...
      foreach(source IN LISTS SOURCES)
        ...
        foreach(ext F90 f90 c)
          ...
          if(EXISTS ${candidate})
            ...
            add_test(NAME ${src_name}_test COMMAND $<TARGET_FILE:${src_name}_test>)
            set_tests_properties(${src_name}_test PROPERTIES LABELS "unit;${target}")
...
function(conditionally_add_test_dir)
  ...
  if(CMAKE_PROJECT_NAME STREQUAL PROJECT_NAME AND BUILD_TESTING)
    if(DEFINED GIT_COMMIT_HASH)
      ...
      file(
        GLOB test_dir_CMAKE_LISTS
        LIST_DIRECTORIES FALSE
        CONFIGURE_DEPENDS "test/CMakeLists.txt"
        )
    endif()
